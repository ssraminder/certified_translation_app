...
const handler: Handler = async (event) => {
  ...
  const { SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, API_KEY } = process.env;

  if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY || !API_KEY) {
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ error: 'Missing Supabase service role key or other environment variables' }),
    };
  }
